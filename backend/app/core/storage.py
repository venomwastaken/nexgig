...
def build_gig_banner_object_key(user_id: uuid.UUID, filename: str) -> str:
    safe_name = filename.replace("/", "_")
    return f"gigs/{user_id}/{uuid.uuid4()}-{safe_name}"


def build_avatar_object_key(user_id: uuid.UUID, filename: str) -> str:
    safe_name = filename.replace("/", "_")
